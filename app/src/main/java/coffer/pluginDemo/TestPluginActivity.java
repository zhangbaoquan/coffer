 ...
 * @RevisionDescription：
 */
public class TestPluginActivity extends BaseDefaultActivity {

    private TextView tv;
    private static Handler mHandler = new Handler();
    private ProgressDialog pd;

    public static final String LOCALDIR= Environment.getExternalStorageDirectory().getPath() +
            File.separator + "apptest-release.apk";
    ...
    public void initView() {
        ...
        ClassLoader loader = PluginMainActivity.class.getClassLoader();
        while (loader != null) {
    ...
    private void load() {
        ...
        if (!apkFile.exists()) {
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    pd.dismiss();
                    Toast.makeText(TestPluginActivity.this, "文件不存在", Toast.LENGTH_LONG);
        ...
        if (!apkFile.canRead()) {
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    pd.dismiss();
                    Toast.makeText(TestPluginActivity.this,
                            "没有读写权限", Toast.LENGTH_LONG);
        ...
        try {
            DexFile dx = DexFile.loadDex(apkFile.getAbsolutePath(), File.createTempFile("opt",
                    "dex", getApplicationContext().getCacheDir()).getPath(), 0);
        ...
        DexClassLoader dexClassLoader =
                new DexClassLoader(apkFile.getAbsolutePath(), getExternalCacheDir().getAbsolutePath(),
                        null, getClassLoader());
        try {
